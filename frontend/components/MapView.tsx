...
export const NYC_HOTSPOTS = [
  { lat: 40.7580, lng: -73.9855, count: 2847, name: 'Times Square' },
  ...
  { lat: 40.7128, lng: -74.0060, count: 1432, name: 'Financial District' },
...
const getRadiusForZoom = (zoom: number, level: string, isPrediction?: boolean): number => {
  let baseRadius = 8;
...
const getHotspotRadius = (count: number, zoom: number): number => {
  ...
  const zoomMultiplier = zoom <= 10 ? 0.3 : zoom <= 12 ? 0.5 : zoom <= 14 ? 0.8 : 1;
...
export default function MapView({ 
  ...
  return (
    ...
    >
      ...
      {/* Hotspots Layer */}
      {showHotspots && NYC_HOTSPOTS.map((hotspot, idx) => (
        <Circle
          key={`hotspot-${idx}`}
        ...
        >
          <Popup>
            <div className="text-sm">
              <div className="font-bold text-red-600 mb-1">⚠️ Accident Hotspot</div>
      ...
      {points.map((point, idx) => {
        ...
        return (
          <CircleMarker
            key={`${point.lat}-${point.lng}-${idx}-${point.isPrediction ? 'pred' : 'data'}`}
              ...
              <div className="text-sm min-w-[200px]">
                ...
                {point.probabilities && (
                  <div className="space-y-1.5 mb-3">
                    <ProbabilityBar label="Low" value={point.probabilities.low} color="#22c55e" />
                    <ProbabilityBar label="Medium" value={point.probabilities.medium} color="#f97316" />
                    <ProbabilityBar label="High" value={point.probabilities.high} color="#ef4444" />
                  </div>
                )}
                ...
                <div className="flex justify-between items-center text-gray-700">
                  ...
                  <span className="font-semibold">{(point.score * 100).toFixed(1)}%</span>
                </div>

                {/* Contributing Factor */}
                {point.contribFactorName && (
                  <div className="mt-1 text-gray-600 text-xs">
                    <span className="font-medium">Factor:</span> {point.contribFactorName}
                  </div>
                )}
